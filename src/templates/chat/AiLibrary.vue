    ...
    <Card class="ai-lib" @dragover="onDragOver" @dragleave="onDragLeave" @drop="onDrop">
        <div class="toolbar">
            ...
            <div class="actions">
                ...
                <input ref="folderInput" type="file" multiple webkitdirectory directory hidden
                    @change="onFolderInputChange" />
            </div>
        </div>

        <Split initialLeftWidth="60%" class="panel" minRightWidth="200px">
            <template #left>
                <div class="left">
                    <div v-if="loading" class="hint">正在加载...</div>
                    <template v-else>
                        <div class="entry" v-for="e in entries" :key="e.name"
                            @dblclick="e.isDir ? openDir(e.name) : openFile(e.name)">
            ...
            <template #right>
                ...
                <div v-if="uploading" class="hint">正在上传 {{ uploadProgress.current }}/{{ uploadProgress.total }} ...
                </div>
                <template v-if="selectedFilePath">
                    <div class="preview-header">
                        <div class="title">预览：{{ selectedFilePath }}</div>
                        <Button :onClick="() => removeFile(selectedFilePath!)"
                            style="min-width: max-content;">删除此文件</Button>
                    </div>
                    <div v-if="!loadingContent" class="preview scrollbar">
                        <pre>{{ fileContent }}</pre>
                    </div>
...
.left {
    border-right: 1px solid var(--border-color, #3333);
    overflow: auto;
    max-height: 100%;
...
.entry {
    ...
    align-items: center;
    padding: 8px 12px;
    border-bottom: 1px dashed var(--border-color, #3333);
    max-width: 100%;
    overflow: hidden;
}
